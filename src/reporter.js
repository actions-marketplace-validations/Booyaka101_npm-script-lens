...
function buildReport(results, { note } = {}) {
  ...
  lines.push('', '_HIGH = spawns processes, runs constructed code, or reads credentials next to network access · MEDIUM = network access · LOW = fs/env only · generated by npm-script-lens_');
  return lines.join('\n');
}
...
${payloadHtml}
${suggestion}
<footer>generated by npm-script-lens · HIGH = spawns processes, runs constructed code, or reads credentials next to network · MEDIUM = network · LOW = fs/env</footer>
</div></body></html>
`;
}
