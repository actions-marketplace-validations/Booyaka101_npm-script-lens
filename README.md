...
- **Obfuscation**: `eval`/`new Function`/`vm` and string-built `require()`s score HIGH, and base64/char-code **literal** payloads are **decoded and re-analyzed**, so the report shows what the hidden code actually does, not just that it hides. A specifier built only from literals is folded back to the module it names, whether it is concatenated, held in a `const`, joined from an array, reversed, or decoded from hex, base64 or char codes, so `require(['child','process'].join('_'))` reports `exec: require('child_process')` as well as the obfuscation. `eval` reached as `globalThis['eval']` or `(0, eval)` counts as `eval`. Payloads assembled only at runtime (downloaded, decrypted, env-derived, built in a loop) remain opaque: flagged, not decoded. Plain variable indirection that cannot be resolved (`require(someVar)`) is deliberately not flagged, it's ubiquitous in bundler output.
